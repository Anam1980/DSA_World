...
public class Main
{
	static HashMap<Integer, Integer>[]mapArr;
	static int snapId;
	
    public static void SnapshotArray(int length) {
		mapArr=new HashMap[length];
		snapId=0;
    }
    ...
    public static int get(int index, int snap_id) {
		...
		//find the last updated val on that index 
		while(snap_id>=0 && mapArr[index].containsKey(snap_id)==false){
			snap_id--;
		}
